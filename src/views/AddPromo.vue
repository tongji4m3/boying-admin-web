<template>
  <div>
        ...
        <el-table-column label="操作" align="center">
          <template slot-scope="scope">
            <el-button
              type="text"
              size="mini"
              @click="addPromo(scope.$index, scope.row)"
              >添加活动</el-button
            >
          </template>
        </el-table-column>
    ...
    <el-dialog
      title="添加活动"
      :visible.sync="addPromoVisiable"
      width="50%"
      :before-close="handleClose"
    >
      <el-form :inline="true" :model="addPromoTable" class="demo-form-inline">
        <el-form-item label="名称">
          <el-input v-model="addPromoTable.name" placeholder="名称"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    //添加活动
    addPromo(index, row) {
      this.addPromoVisiable = true;
      this.addPromoTable.seatId = row.id;
    },
    //添加座次提交
    async addSeatSubmit() {
