...
export default {
  ...
  methods: {
    async getShowDetail() {
      ...
      console.log(id);
      try {
        const res = await axios.post(`${api.API_URL}/show/detail/` + id, {
          headers: {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(async (valid) => {
        if (!valid || this.ruleForm.poster == "") {
          ...
          this.$message.error("更新失败，请完整填写表单");
          return false;
        } else {
          try {
            console.log("this.ruleForm", this.ruleForm);
            const res = await axios.post(
              `${api.API_URL}/show/update/` + this.ruleForm.id,
              {
            ...
            console.log("/show/update", res);
            if (res.data.code == 200) {
              this.$message.success("更新演出成功,即将跳转演出界面");
              // 等待不起作用
              setTimeout(() => {
                this.loading = false;
              }, 500);
            ...
            console.log(err);
            this.$message.error("更新失败，数据填写错误");
          }
        }
      });
    },
